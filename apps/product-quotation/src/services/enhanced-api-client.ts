...
export class EnhancedApiClient extends ProductQuotationApiClient {
  ...
  async syncLocalDataToBackend(): Promise<{
    success: number;
    failed: number;
    errors: string[];
  }> {
    ...
    for (const quotation of localQuotations) {
      try {
        ...
        results.failed++;
        results.errors.push(
          `Failed to sync quotation ${quotation.quotationNumber}: ${error}`
        );
      }
    }
    ...
    try {
      ...
      console.log(
        `Backend availability: ${
          this.isBackendAvailable ? 'Available' : 'Unavailable'
        }`
      );
      return this.isBackendAvailable;
    } catch (error) {
