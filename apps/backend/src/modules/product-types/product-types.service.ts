...
export class ProductTypesService {
  ...
  async create(createProductTypeDto: CreateMasterDataDto): Promise<ProductTypes> {
  ...
  async update(id: number, updateProductTypeDto: UpdateMasterDataDto): Promise<ProductTypes> {
  ...
  async search(term: string): Promise<ProductTypes[]> {
    return await this.productTypeRepository
      ...
      .andWhere('(productType.name LIKE :term OR productType.code LIKE :term)', {
        term: `%${term}%`,
      })
